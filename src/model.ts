...
export function geminiInquiryReply(dataDir:string) {
  return async(inquiry:InquiryCase)=>{
    const lines=inquiry.lines.map(line=>{
      ...
      return {itemText:line.itemText,quantity:line.quantity,availability};
    });
    const input=JSON.stringify({language:inquiry.replyLanguage,senderName:inquiry.senderName,company:inquiry.customerText,intent:inquiry.intent,
      condition:inquiry.condition,requestedDate:inquiry.requestedDate,deliveryAddress:inquiry.deliveryAddress,lines});
