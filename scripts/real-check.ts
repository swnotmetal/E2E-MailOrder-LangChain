...
try {
  ...
  const [customers,itemMatches,addresses,duplicates,prices]=await Promise.all([
    erp.findCustomers(draft.customer),Promise.all(draft.lines.map((line:any)=>erp.findItems(line.item))),erp.addresses(draft.customer),erp.duplicates(draft),
  ...
  console.log(JSON.stringify({id,status:state.status,draft,issues:state.issues,
    ...
    exactCustomerMatch:customers.some(c=>c.name===draft.customer),exactItemMatch:draft.lines.every((line:any,index:number)=>itemMatches[index].some((i:{name:string})=>i.name===line.item)),
