...
2026-09-23：真实 Gemini 自然语言邮件抽取已验证，包括公司、联系人、明确送货地址和条件购买路由；询价流程还会在只读 ERP 查询后用一次独立请求生成同语种回复。该请求只接收结构化虚构结果，必须保留商品、数量及原文交付日期，并停在人工审核。一条完整 LangGraph 根 trace 已验证包含抽取与回复 LLM span、两个只读库存 Tool span和人工 interrupt；人工批准也已作为 LangSmith feedback 读回。此前旧供应商一次请求返回 401，保留预算记录，不将未知费用写成已证实的零。
